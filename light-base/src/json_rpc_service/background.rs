        ...
        match wake_up_reason {
            ...
            WakeUpReason::IncomingJsonRpcRequest(request_json) => {
                ...
                match request_parsed {
                    ...
                    methods::MethodCall::eth_blockNumber {} => {
                        // Return the best block number directly from the
                        // subscription state, no runtime call needed.
                        if let RuntimeServiceSubscription::Active {
                            current_best_block,
                            pinned_blocks,
                            ..
                        ...
                        {
                            let block_number = pinned_blocks
                                .get(current_best_block)
                                .and_then(|b| {
                                    header::decode(
                            ...
                            let _ = me
                                .responses_tx
                                .send(
                                    methods::Response::eth_blockNumber(
                            ...
                            let _ = me
                                ...
                                .send(
                                    methods::Response::eth_blockNumber(
                                        methods::EthQuantity::from_u64(0),
                                    )
                                    .to_json_response(request_id_json),
                                )
                                .await;
                    ...
                    methods::MethodCall::web3_clientVersion {} => {
                        let _ = me
                            .responses_tx
                            .send(
                                methods::Response::web3_clientVersion(
                    ...
                    methods::MethodCall::eth_gasPrice {} => {
                        me.multistage_requests_to_advance.push_back((
                            request_id_json.to_owned(),
                            MultiStageRequestStage::BlockHashNotKnown,
                            MultiStageRequestTy::EthGasPrice,
                        ));
                    }

                    methods::MethodCall::eth_getBalance {
                        address: methods::EthAddress(address),
                        block,
                    } => {
                        let stage = eth_block_param_to_stage(block, me.genesis_block_hash);
                        match stage {
                            Ok(stage) => {
                                me.multistage_requests_to_advance.push_back((
                                    request_id_json.to_owned(),
                        ...
                        match stage {
                            Ok(stage) => {
                                me.multistage_requests_to_advance.push_back((
                                    request_id_json.to_owned(),
                                    stage,
                                    MultiStageRequestTy::EthGetTransactionCount { address },
                                ));
                            }
                            Err(err_response) => {
                                let _ = me
                        ...
                        match stage {
                            Ok(stage) => {
                                me.multistage_requests_to_advance.push_back((
                                    request_id_json.to_owned(),
                                    stage,
                                    MultiStageRequestTy::EthGetCode { address },
                                ));
                            }
                            Err(err_response) => {
                                let _ = me
                    ...
                    methods::MethodCall::eth_getStorageAt {
                        address: methods::EthAddress(address),
                        position: methods::EthQuantity(position),
                        block,
                    } => {
                        let stage = eth_block_param_to_stage(block, me.genesis_block_hash);
                        match stage {
                            Ok(stage) => {
                                me.multistage_requests_to_advance.push_back((
                                    request_id_json.to_owned(),
                                    stage,
                                    MultiStageRequestTy::EthGetStorageAt { address, position },
                                ));
                            }
                            Err(err_response) => {
                                let _ = me
                    ...
                    methods::MethodCall::eth_call {
                        ...
                        match stage {
                            Ok(stage) => {
                                let call_params = eth_call_object_to_params(&transaction, false);
                                me.multistage_requests_to_advance.push_back((
                                    request_id_json.to_owned(),
                                    stage,
                                    MultiStageRequestTy::EthCall { call_params },
                                ));
                            }
                            Err(err_response) => {
                                let _ = me
                    ...
                    methods::MethodCall::eth_estimateGas {
                        ...
                        match stage {
                            Ok(stage) => {
                                let call_params = eth_call_object_to_params(&transaction, true);
                                me.multistage_requests_to_advance.push_back((
                                    request_id_json.to_owned(),
                                    stage,
                                    MultiStageRequestTy::EthEstimateGas { call_params },
                                ));
                            }
                            Err(err_response) => {
                                let _ = me
            ...
            WakeUpReason::AdvanceMultiStageRequest {
                ...
                request_ty:
                    ...
                    | MultiStageRequestTy::EthCall { .. }
                    | MultiStageRequestTy::EthEstimateGas { .. }
                    | MultiStageRequestTy::EthGasPrice
                    | MultiStageRequestTy::EthGetTransactionCount { .. }),
            } => {
                    ...
                    let (function_name, required_api_version, parameters_vectored, request_update) =
                        match request_ty {
                            ...
                            MultiStageRequestTy::EthGetBalance { address } => (
                                ...
                                Some((json_rpc::ethereum::API_NAME.to_owned(), json_rpc::ethereum::API_VERSION_RANGE)),
                                json_rpc::ethereum::account_basic_parameters(&address),
                                RuntimeCallRequestInProgress::EthAccountBasicForBalance,
                            ),
                            MultiStageRequestTy::EthGetTransactionCount { address } => (
                                json_rpc::ethereum::ACCOUNT_BASIC_FUNCTION_NAME.to_owned(),
                                Some((json_rpc::ethereum::API_NAME.to_owned(), json_rpc::ethereum::API_VERSION_RANGE)),
                                json_rpc::ethereum::account_basic_parameters(&address),
                                RuntimeCallRequestInProgress::EthAccountBasicForNonce,
                            ),
                            MultiStageRequestTy::EthGetCode { address } => (
                                json_rpc::ethereum::ACCOUNT_CODE_AT_FUNCTION_NAME.to_owned(),
                                Some((json_rpc::ethereum::API_NAME.to_owned(), json_rpc::ethereum::API_VERSION_RANGE)),
                                json_rpc::ethereum::account_code_at_parameters(&address),
                                RuntimeCallRequestInProgress::EthAccountCode,
                            ),
                            MultiStageRequestTy::EthGetStorageAt { address, position } => (
                                json_rpc::ethereum::STORAGE_AT_FUNCTION_NAME.to_owned(),
                                Some((json_rpc::ethereum::API_NAME.to_owned(), json_rpc::ethereum::API_VERSION_RANGE)),
                                json_rpc::ethereum::storage_at_parameters(&address, &position),
                                RuntimeCallRequestInProgress::EthStorageAt,
                            ),
                            MultiStageRequestTy::EthCall { call_params } => (
                                json_rpc::ethereum::CALL_FUNCTION_NAME.to_owned(),
                                Some((json_rpc::ethereum::API_NAME.to_owned(), json_rpc::ethereum::API_VERSION_RANGE)),
                                call_params,
                                RuntimeCallRequestInProgress::EthCall,
                            ),
                            MultiStageRequestTy::EthEstimateGas { call_params } => (
                                json_rpc::ethereum::CALL_FUNCTION_NAME.to_owned(),
                                Some((json_rpc::ethereum::API_NAME.to_owned(), json_rpc::ethereum::API_VERSION_RANGE)),
                                call_params,
                                RuntimeCallRequestInProgress::EthEstimateGas,
                            ),
                            MultiStageRequestTy::EthGasPrice => (
                                json_rpc::ethereum::GAS_PRICE_FUNCTION_NAME.to_owned(),
                                Some((json_rpc::ethereum::API_NAME.to_owned(), json_rpc::ethereum::API_VERSION_RANGE)),
                                json_rpc::ethereum::gas_price_parameters(),
                                RuntimeCallRequestInProgress::EthGasPrice,
                            ),
                            _ => unreachable!(),
                        };
            ...
            WakeUpReason::Event(Event::LegacyApiFunctionRuntimeCallResult {
                ...
                match (result, request) {
                    ...
                    (Ok(result), RuntimeCallRequestInProgress::EthChainId) => {
                        match json_rpc::ethereum::decode_chain_id(&result.output) {
                            Ok(chain_id) => {
                                let _ = me
                                    .responses_tx
                                    .send(
                                        methods::Response::eth_chainId(
                                            methods::EthQuantity::from_u64(chain_id),
                                        )
                                        .to_json_response(&request_id_json),
                                    )
                                    .await;
                            ...
                            Err(error) => {
                                let _ = me
                                    ...
                                    .send(parse::build_error_response(
                                        ...
                                        parse::ErrorResponse::ServerError(
                                            -32000,
                                            &format!("Failed to decode chain_id: {error}"),
                                        ),
                                        None,
                                    ))
                                    .await;
                    ...
                    (Ok(result), RuntimeCallRequestInProgress::NetVersion) => {
                        match json_rpc::ethereum::decode_chain_id(&result.output) {
                            Ok(chain_id) => {
                                let _ = me
                                    .responses_tx
                                    .send(
                                        methods::Response::net_version(
                    ...
                    (Ok(result), RuntimeCallRequestInProgress::EthAccountBasicForBalance) => {
                        match json_rpc::ethereum::decode_account_basic(&result.output) {
                            Ok(info) => {
                                let _ = me
                                    .responses_tx
                                    .send(
                                        methods::Response::eth_getBalance(
                                            methods::EthQuantity::from_le_bytes(info.balance),
                                        )
                                        .to_json_response(&request_id_json),
                                    )
                                    .await;
                            ...
                            Err(error) => {
                                let _ = me
                                    ...
                                    .send(parse::build_error_response(
                                        ...
                                        parse::ErrorResponse::ServerError(
                                            -32000,
                                            &format!("Failed to decode account_basic: {error}"),
                                        ),
                                        None,
                                    ))
                                    .await;
                    ...
                    (Ok(result), RuntimeCallRequestInProgress::EthAccountBasicForNonce) => {
                        match json_rpc::ethereum::decode_account_basic(&result.output) {
                            Ok(info) => {
                                let _ = me
                                    .responses_tx
                                    .send(
                                        methods::Response::eth_getTransactionCount(
                                            methods::EthQuantity::from_le_bytes(info.nonce),
                                        )
                                        .to_json_response(&request_id_json),
                                    )
                                    .await;
                    ...
                    (Ok(result), RuntimeCallRequestInProgress::EthAccountCode) => {
                        match json_rpc::ethereum::decode_account_code(&result.output) {
                            Ok(code) => {
                                let _ = me
                                    .responses_tx
                                    .send(
                                        methods::Response::eth_getCode(
                                            methods::HexString(code),
                                        )
                                        .to_json_response(&request_id_json),
                                    )
                                    .await;
                            ...
                            Err(error) => {
                                let _ = me
                                    ...
                                    .send(parse::build_error_response(
                                        ...
                                        parse::ErrorResponse::ServerError(
                                            -32000,
                                            &format!("Failed to decode account_code: {error}"),
                                        ),
                                        None,
                                    ))
                                    .await;
                    ...
                    (Ok(result), RuntimeCallRequestInProgress::EthStorageAt) => {
                        match json_rpc::ethereum::decode_storage_at(&result.output) {
                            Ok(value) => {
                                let _ = me
                                    .responses_tx
                                    .send(
                                        methods::Response::eth_getStorageAt(
                                            methods::HexString(value.to_vec()),
                                        )
                                        .to_json_response(&request_id_json),
                                    )
                                    .await;
                            ...
                            Err(error) => {
                                let _ = me
                                    ...
                                    .send(parse::build_error_response(
                                        ...
                                        parse::ErrorResponse::ServerError(
                                            -32000,
                                            &format!("Failed to decode storage_at: {error}"),
                                        ),
                                        None,
                                    ))
                                    .await;
                    ...
                    (Ok(result), RuntimeCallRequestInProgress::EthCall) => {
                        match json_rpc::ethereum::decode_call_result(&result.output) {
                            Ok(call_result) => {
                                let _ = me
                                    .responses_tx
                                    .send(
                                        methods::Response::eth_call(
                                            methods::HexString(call_result.return_data),
                                        )
                                        .to_json_response(&request_id_json),
                                    )
                                    .await;
                            }
                            Err(json_rpc::ethereum::DecodeError::EvmReverted { return_data }) => {
                                let _ = me
                                    .responses_tx
                                    .send(parse::build_error_response(
                                        &request_id_json,
                                        ...
                                        Some(&format!("\"0x{}\"", hex::encode(&return_data))),
                                    ))
                                    .await;
                            }
                            Err(error) => {
                                let _ = me
                                    ...
                                    .send(parse::build_error_response(
                                        ...
                                        parse::ErrorResponse::ServerError(
                                            -32000,
                                            &format!("Failed to execute eth_call: {error}"),
                                        ),
                                        None,
                                    ))
                                    .await;
                    ...
                    (Ok(result), RuntimeCallRequestInProgress::EthEstimateGas) => {
                        match json_rpc::ethereum::decode_call_result(&result.output) {
                            Ok(call_result) => {
                                let _ = me
                                    .responses_tx
                                    .send(
                                        methods::Response::eth_estimateGas(
                                            methods::EthQuantity::from_le_bytes(call_result.used_gas),
                                        )
                                        .to_json_response(&request_id_json),
                                    )
                                    .await;
                            ...
                            Err(error) => {
                                let _ = me
                                    ...
                                    .send(parse::build_error_response(
                                        ...
                                        parse::ErrorResponse::ServerError(
                                            -32000,
                                            &format!("Failed to execute eth_estimateGas: {error}"),
                                        ),
                                        None,
                                    ))
                                    .await;
                    ...
                    (Ok(result), RuntimeCallRequestInProgress::EthGasPrice) => {
                        match json_rpc::ethereum::decode_gas_price(&result.output) {
                            Ok(price) => {
                                let _ = me
                                    .responses_tx
                                    .send(
                                        methods::Response::eth_gasPrice(
                                            methods::EthQuantity::from_le_bytes(price),
                                        )
                                        .to_json_response(&request_id_json),
                                    )
                                    .await;
                            ...
                            Err(error) => {
                                let _ = me
                                    ...
                                    .send(parse::build_error_response(
                                        ...
                                        parse::ErrorResponse::ServerError(
                                            -32000,
                                            &format!("Failed to decode gas_price: {error}"),
                                        ),
                                        None,
                                    ))
                                    .await;
...
/// Converts an `EthBlockParameter` to a `MultiStageRequestStage`.
fn eth_block_param_to_stage(
    block: Option<methods::EthBlockParameter>,
    genesis_block_hash: [u8; 32],
) -> Result<MultiStageRequestStage, String> {
    match block {
        None | Some(methods::EthBlockParameter::Latest) | Some(methods::EthBlockParameter::Pending) => {
            Ok(MultiStageRequestStage::BlockHashNotKnown)
        }
        Some(methods::EthBlockParameter::Earliest) => {
            Ok(MultiStageRequestStage::BlockHashKnown {
                block_hash: genesis_block_hash,
            })
        }
        Some(methods::EthBlockParameter::Number(_)) => {
            Err("Querying by specific block number is not supported yet".to_owned())
        }
    }
}

/// Converts an `EthCallObject` into pre-encoded SCALE parameters for the
/// `EthereumRuntimeRPCApi_call` runtime API.
fn eth_call_object_to_params(tx: &methods::EthCallObject, estimate: bool) -> Vec<u8> {
    let zero_value = [0u8; 32];
    let default_gas_limit = {
        // Default gas limit: 15_000_000 (0xE4E1C0)
        let mut g = [0u8; 32];
        let gas_bytes = 15_000_000u64.to_le_bytes();
        g[..8].copy_from_slice(&gas_bytes);
        g
    };

    json_rpc::ethereum::call_parameters(
        tx.from.as_ref().map(|a| &a.0),
        tx.to.as_ref().map(|a| &a.0),
        tx.data.as_ref().map(|d| d.0.as_slice()).unwrap_or(&[]),
        tx.value.as_ref().map(|v| &v.0).unwrap_or(&zero_value),
        tx.gas.as_ref().map(|g| &g.0).unwrap_or(&default_gas_limit),
        tx.max_fee_per_gas.as_ref().map(|v| &v.0),
        tx.max_priority_fee_per_gas.as_ref().map(|v| &v.0),
        tx.nonce.as_ref().map(|v| &v.0),
        estimate,
    )
}
